...
NotifierProvider<QueryNotifier<List<Post>>, QueryState<List<Post>>>
    userPostsQueryProvider(int userId) {
...
final userPostsAsyncQueryProviderFamily =
    asyncQueryProviderFamily<List<Post>, int>(
...
final createPostMutationProvider = createProvider<Post, Map<String, dynamic>>(
  ...
  onMutate: (ref, variables) async {
    ...
    final infiniteQueryEntry =
        queryClient.getCacheEntry<List<PostPage>>('posts-infinite');
    if (infiniteQueryEntry?.hasData ?? false) {
  ...
  onError: (ref, variables, error, stackTrace) {
    ...
    if (infiniteQueryEntry?.hasData ?? false) {
      ...
      if (currentPages.isNotEmpty) {
        ...
        final rollbackPosts = firstPage.posts.where((p) => p.id >= 0).toList();
...
final updatePostMutationProvider =
    updateProvider<Post, Map<String, dynamic>, int>(
  name: 'update-post',
  mutationFn: (ref, variables, postId) =>
      ApiService.updatePost(postId, variables),
  onMutate: (ref, variables, postId) async {
    ...
    final userPostsEntry = queryClient
        .getCacheEntry<List<Post>>('user-posts-${variables['userId']}');
    if (userPostsEntry?.hasData ?? false) {
      ...
      if (postIndex != -1) {
        ...
        queryClient.setQueryData(
            'user-posts-${variables['userId']}', currentUserPosts);
      }
    }
  ...
  onSuccess: (ref, post, variables, postId) {
    ...
    final userPostsEntry =
        queryClient.getCacheEntry<List<Post>>('user-posts-${post.userId}');
    if (userPostsEntry?.hasData ?? false) {
      final currentUserPosts = List<Post>.from(userPostsEntry!.data!);
      final postIndex = currentUserPosts.indexWhere((p) => p.id == postId);
...
final deletePostMutationProvider = deleteProviderWithParam<void, int>(
  ...
  onMutate: (ref, postId) async {
    ...
    if (deletedPost != null) {
      final userPostsEntry = queryClient
          .getCacheEntry<List<Post>>('user-posts-${deletedPost.userId}');
      if (userPostsEntry?.hasData ?? false) {
        final currentUserPosts = List<Post>.from(userPostsEntry!.data!);
        currentUserPosts.removeWhere((p) => p.id == postId);
        queryClient.setQueryData(
            'user-posts-${deletedPost.userId}', currentUserPosts);
      }
    }
