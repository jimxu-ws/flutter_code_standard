...
class _HomeScreenState extends ConsumerState<HomeScreen> {
  ...
  final List<TabData> _tabs = const [
    ...
    TabData(text: 'UsersDisk', icon: Icons.people),
...
class InfiniteTabBar extends ConsumerWidget {
  const InfiniteTabBar({
    required this.tabs, required this.currentIndex, required this.onTabTapped, super.key,
  });
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    properties.add(IterableProperty<TabData>('tabs', tabs));
    properties.add(IntProperty('currentIndex', currentIndex));
    properties.add(ObjectFlagProperty<ValueChanged<int>>.has('onTabTapped', onTabTapped));
  }
}

class UsersDiskTab extends ConsumerWidget {
  const UsersDiskTab({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final usersState = ref.watch(usersDiskQueryProvider);
    ...
    return usersState.when(
      ...
      success: (users) => RefreshIndicator(
        onRefresh: () async {
          await ref.read(usersDiskQueryProvider.notifier).refetch();
        },
        child: Column(
          children: [
            // Comparison button
      ...
      error: (error, stackTrace) => Center(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            ElevatedButton(
              onPressed: () => ref.read(usersDiskQueryProvider.notifier).refetch(),
...
class AsyncUserListTile extends ConsumerWidget {
  const AsyncUserListTile({required this.user, super.key});
