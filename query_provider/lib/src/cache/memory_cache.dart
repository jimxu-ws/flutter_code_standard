...
const defaultStaleTime = Duration(minutes: 30);

/// In-memory cache implementation for query data
class MemoryQueryCache extends QueryCache {
  /// Create a new memory query cache
  factory MemoryQueryCache() => _instance;
  MemoryQueryCache._internal() {
    _startCleanupTimer();
  }
  static final MemoryQueryCache _instance = MemoryQueryCache._internal();

  /// Maximum number of entries to keep in cache
  final int maxSize = QueryGlobalOptions().cacheMaxSize;

  /// Default cache time for entries without specific options
  final Duration defaultCacheTime = QueryGlobalOptions().defaultMemoryCacheTime;
  ...
  QueryCacheEntry<T>? get<T>(String key, {JsonParser<T>? jsonParser}) {
    ...
    if (entry.shouldEvict) {
      Log.info('Evicting entry for key $key in query cache');
      _cache.remove(key);
      _evictionCount++;
      ...
      return Duration(
        milliseconds: (timeUntilExpiration.inMilliseconds + 60000).clamp(
          ...
          shortestCacheTime.inMilliseconds ~/
              2, // Maximum half of shortest cache time
        ),
      );
    }
  ...
  void _emitEvent(QueryCacheEventType type, String key,
    ...
    Log.info('Emitting cache event for key $key in query cache, type: $type');
