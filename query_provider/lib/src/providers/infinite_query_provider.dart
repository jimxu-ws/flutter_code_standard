...
sealed class InfiniteQueryState<T> {
  ...
  bool get hasData =>
      this is InfiniteQuerySuccess<T> || this is InfiniteQueryRefetching<T> || this is InfiniteQueryFetchingNextPage<T> || this is InfiniteQueryFetchingPreviousPage<T>;
  
  bool get isSuccess => this is InfiniteQuerySuccess<T>;
  ...
  bool get isFetchingPreviousPage =>
      this is InfiniteQueryFetchingPreviousPage<T>;
...
final class InfiniteQueryFetchingNextPage<T> extends InfiniteQueryState<T> {
  ...
  @override
  String toString() =>
      'InfiniteQueryFetchingNextPage<$T>(pages: ${pages.length})';
}
...
final class InfiniteQueryFetchingPreviousPage<T> extends InfiniteQueryState<T> {
  ...
  @override
  String toString() =>
      'InfiniteQueryFetchingPreviousPage<$T>(pages: ${pages.length})';
}

/// Modern FamilyNotifier for managing infinite query state
class InfiniteQueryNotifier<T, TPageParam>
    extends Notifier<InfiniteQueryState<T>> {
  ...
  void _setupDispose() {
    ref.onDispose(() {
      ...
      Log.info('InfiniteQueryNotifier disposed for key $queryKey');
    });
  }
  ...
  Future<void> _fetchFirstPage() async {
    ...
    Log.info('Fetching first page for infinite query key $queryKey');

    // Check cache first
    final cachedEntry = _getCachedEntry();
    if (cachedEntry != null && !cachedEntry.isStale && cachedEntry.hasData) {
      Log.info('Using cached data for infinite query key $queryKey');
    ...
    if (options.keepPreviousData &&
        ...
        cachedEntry.hasData) {
      Log.info(
          'Using stale cached data with keepPreviousData for infinite query key $queryKey');
  ...
  void _fetchFirstPageInBackground() {
    _fetchFirstPageCore().then((result) {
      ...
      Log.info(
          'Background fetch failed for infinite query key $queryKey: $error');
  ...
  Future<void> refetch() async {
    ...
    if (currentState is InfiniteQuerySuccess<T>) {
      ...
      try {
        ...
        // if (options.keepPreviousData) {
        Log.info(
            'Refetch failed but keeping previous data for infinite query key $queryKey: $error');
  ...
  void _onWindowBlured() {
    if(isMobile()){
      Log.info('App paused in infinite query notifier');
  ...
  void _onWindowFocused() {
    Log.info('Window focused in infinite query notifier');
  ...
  /// Get cached entry
  QueryCacheEntry<InfiniteQueryCacheData<T>>? _getCachedEntry() =>
      _cache.get<InfiniteQueryCacheData<T>>(queryKey);
  ...
  void _setupCacheListener() {
    _cache.addListener<InfiniteQueryCacheData<T>>(queryKey, (entry) {
      Log.info(
          'Cache listener called for key $queryKey in infinite query notifier');
      if ((entry?.hasData ?? false) &&
          !(state.hasData && listEquals(entry!.data!.pages, state.pages))) {
        Log.info(
            'Cache data changed for key $queryKey in infinite query notifier');
        ...
        Log.info(
            'Cache entry removed for key $queryKey in infinite query notifier');
...
NotifierProvider<InfiniteQueryNotifier<T, TPageParam>, InfiniteQueryState<T>>
    infiniteQueryProvider<T, TPageParam>(
        ...
        NotifierProvider<InfiniteQueryNotifier<T, TPageParam>,
...
class InfiniteQueryNotifierAutoDispose<T, TPageParam>
    extends AutoDisposeNotifier<InfiniteQueryState<T>> {
  ...
  void _setupDispose() {
    ref.onDispose(() {
      ...
      Log.info('InfiniteQueryNotifierAutoDispose disposed for key $queryKey');
    });
  }
  ...
  Future<void> fetchNextPage() async {
    ...
    final nextPageParam =
        options.getNextPageParam(currentState.pages.last, currentState.pages);
    if (nextPageParam == null) {
      return;
    }

    Log.info('Fetching next page for infinite query key $queryKey');
  ...
  Future<void> fetchPreviousPage() async {
    ...
    final previousPageParam = options.getPreviousPageParam!(
        currentState.pages.first, currentState.pages);
    if (previousPageParam == null) {
      return;
    }

    Log.info('Fetching previous page for infinite query key $queryKey');
  ...
  Future<void> refetch() async {
    ...
    try {
      ...
      // Fetch the same number of pages as currently loaded
      final pagesToLoad = currentState is InfiniteQuerySuccess<T>
...
/// );
/// ```
AutoDisposeNotifierProvider<InfiniteQueryNotifierAutoDispose<T, TPageParam>,
    InfiniteQueryState<T>> infiniteQueryProviderAutoDispose<T, TPageParam>({
  required String name,
  required QueryFunctionWithParamsWithRef<T, TPageParam> queryFn,
  required TPageParam initialPageParam,
  required InfiniteQueryOptions<T, TPageParam> options,
    ...
    NotifierProvider.autoDispose<
        InfiniteQueryNotifierAutoDispose<T, TPageParam>, InfiniteQueryState<T>>(
...
class InfiniteQueryResult<T> {
  ...
  bool get hasNextPage => switch (state) {
        ...
        final InfiniteQueryFetchingPreviousPage<T> fetching =>
  ...
  bool get hasPreviousPage => switch (state) {
        ...
        final InfiniteQueryFetchingNextPage<T> fetching =>
          fetching.hasPreviousPage,
        final InfiniteQueryFetchingPreviousPage<T> fetching =>
          fetching.hasPreviousPage,
        _ => false,
      };
...
/// Modern FamilyNotifier for managing infinite query state with parameters
class InfiniteQueryNotifierFamily<T, TPageParam, P>
    extends FamilyNotifier<InfiniteQueryState<T>, P> {
  InfiniteQueryNotifierFamily({
    required this.queryFn,
    required this.options,
    required this.initialPageParam,
    required this.queryKey,
  ...
  @override
  InfiniteQueryState<T> build(P arg) {
    _initialize(arg);
    _setupDispose(arg);
    return const InfiniteQueryIdle();
  }

  void _setupDispose(P arg) {
    ...
    ref.onDispose(() {
      ...
      Log.info('InfiniteQueryNotifierFamily disposed for key $paramKey');
    });
  }
  ...
  /// Fetch the first page
  Future<void> _fetchFirstPage(P arg) async {
    if (!options.enabled) {
      return;
    }

    final paramKey = '$queryKey-$arg';
    Log.info('Fetching first page for infinite query key $paramKey');

    // Check cache first
    final cachedEntry = _getCachedEntry(paramKey);
    if (cachedEntry != null && !cachedEntry.isStale && cachedEntry.hasData) {
      Log.info('Using cached data for infinite query key $paramKey');
      final cacheData = cachedEntry.data!;
      _safeState(InfiniteQuerySuccess<T>(
        pages: cacheData.pages,
        hasNextPage: cacheData.hasNextPage,
    ...
    if (options.keepPreviousData &&
        ...
        cachedEntry.hasData) {
      Log.info(
          'Using stale cached data with keepPreviousData for infinite query key $paramKey');
      final staleData = cachedEntry.data!;
      _safeState(InfiniteQuerySuccess<T>(
        pages: staleData.pages,
        hasNextPage: staleData.hasNextPage,
    ...
    try {
      ...
      _setCachedEntry(
          paramKey,
          QueryCacheEntry<InfiniteQueryCacheData<T>>(
            data: cacheData,
            fetchedAt: now,
            cacheTime: options.cacheTime,
            staleTime: options.staleTime,
  ...
  void _fetchFirstPageInBackground(P arg) {
    _fetchFirstPageCore(arg).then((result) {
      if (result != null) {
        final (pages, hasNextPage, hasPreviousPage, fetchedAt) = result;
        _safeState(InfiniteQuerySuccess<T>(
          pages: pages,
      ...
      // On error, keep the current stale data but log the error
      Log.info(
          'Background fetch failed for infinite query key $queryKey-$arg: $error');
      // Don't update state on error to keep showing stale data
    });
  }

  /// Core fetch logic that can be reused
  Future<(List<T>, bool, bool, DateTime)?> _fetchFirstPageCore(P arg) async {
    try {
      final firstPage = await queryFn(arg)(ref, initialPageParam);
      final now = DateTime.now();
      final pages = [firstPage];
  ...
  Future<void> refetch() async {
    ...
    if (currentState is InfiniteQuerySuccess<T>) {
      ...
      try {
        ...
        // Keep showing the current data
        Log.info(
            'Refetch failed but keeping previous data for infinite query key $queryKey-$arg: $error');
        _safeState(InfiniteQueryError(error, stackTrace: stackTrace));
      }
    } else {
      await _fetchFirstPage(arg);
    }
  }
  ...
  void _onWindowBlured() {
    if(isMobile()){
      Log.info('App paused in infinite query notifier family');
      // Mark refetching as paused
      _isRefetchPaused = true;
    }
  }
  ...
  void _onWindowFocused() {
    Log.info('Window focused in infinite query notifier family');
    // Resume refetching and check if we need to refetch stale data
    _isRefetchPaused = false;
  ...
  /// Get cached entry
  QueryCacheEntry<InfiniteQueryCacheData<T>>? _getCachedEntry(String key) =>
      _cache.get<InfiniteQueryCacheData<T>>(key);

  /// Set cached entry
  void _setCachedEntry(
      String key, QueryCacheEntry<InfiniteQueryCacheData<T>> entry) {
  ...
  void _setupCacheListener(String key) {
    _cache.addListener<InfiniteQueryCacheData<T>>(key, (entry) {
      Log.info(
          'Cache listener called for key $key in infinite query notifier family');
      if ((entry?.hasData ?? false) &&
          !(state.hasData && listEquals(entry!.data!.pages, state.pages))) {
        Log.info(
            'Cache data changed for key $key in infinite query notifier family');
        // Update state when cache data changes externally (e.g., optimistic updates)
        final cacheData = entry!.data!;
        _safeState(InfiniteQuerySuccess(
          pages: cacheData.pages,
        ...
        Log.info(
            'Cache entry removed for key $key in infinite query notifier family');
        // Cache entry was removed, reset to idle
        if (options.onCacheEvicted != null) {
...
/// Auto-dispose FamilyNotifier for managing infinite query state with parameters
class InfiniteQueryNotifierFamilyAutoDispose<T, TPageParam, P>
    extends AutoDisposeFamilyNotifier<InfiniteQueryState<T>, P> {
  InfiniteQueryNotifierFamilyAutoDispose({
    required this.queryFn,
    required this.options,
    required this.initialPageParam,
    required this.queryKey,
  ...
  void _setupDispose(P arg) {
    ...
    ref.onDispose(() {
      ...
      Log.info(
          'InfiniteQueryNotifierFamilyAutoDispose disposed for key $paramKey');
    });
  }
  ...
  Future<(List<T>, bool, bool, DateTime)?> _fetchFirstPageCore(P arg) async {
    try {
      ...
      if (_retryCount < options.retry) {
        _retryCount++;
        await Future<void>.delayed(options.retryDelay);
        return _fetchFirstPageCore(arg);
      }
  ...
  Future<void> fetchNextPage() async {
    ...
    final paramKey = '$queryKey-$arg';
    Log.info('Fetching next page for infinite query key $paramKey');
  ...
  Future<void> fetchPreviousPage() async {
    ...
    final paramKey = '$queryKey-$arg';
    Log.info('Fetching previous page for infinite query key $paramKey');
  ...
  void _onWindowBlured() {
    if(isMobile()){
      Log.info('App paused in infinite query notifier family auto dispose');
      // Mark refetching as paused
      _isRefetchPaused = true;
    }
  }
  ...
  void _onWindowFocused() {
    Log.info('Window focused in infinite query notifier family auto dispose');
    // Resume refetching and check if we need to refetch stale data
    _isRefetchPaused = false;
  ...
  void _setupCacheListener(String key) {
    _cache.addListener<InfiniteQueryCacheData<T>>(key, (entry) {
      Log.info(
          'Cache listener called for key $key in infinite query notifier family auto dispose');
      if ((entry?.hasData ?? false) &&
          !(state.hasData && listEquals(entry!.data!.pages, state.pages))) {
        Log.info(
            'Cache data changed for key $key in infinite query notifier family auto dispose');
        // Update state when cache data changes externally (e.g., optimistic updates)
        final cacheData = entry!.data!;
        _safeState(InfiniteQuerySuccess(
          pages: cacheData.pages,
        ...
        Log.info(
            'Cache entry removed for key $key in infinite query notifier family auto dispose');
        // Cache entry was removed, reset to idle
        if (options.onCacheEvicted != null) {
...
/// 🔥 Full-featured Notifier-based infinite query provider with parameters
///
/// **Use this when:**
...
/// Example:
/// ```dart
/// final postsProvider = infiniteQueryProviderFamily<Post, int, String>(
///   name: 'posts',
...
///
/// // Usage:
/// final postsResult = ref.readInfiniteQueryResultFamily(postsProvider, 'technology');
/// postsResult.state.when(
///   idle: () => Text('Tap to load'),
///   loading: () => CircularProgressIndicator(),
///   success: (pages, hasNextPage, hasPreviousPage, fetchedAt) =>
...
/// );
/// ```
NotifierProviderFamily<InfiniteQueryNotifierFamily<T, TPageParam, P>,
    InfiniteQueryState<T>, P> infiniteQueryProviderFamily<T, TPageParam, P>({
  required String name,
  required QueryFunctionWithParamsWithRef<T, TPageParam> Function(P) queryFn,
  required TPageParam initialPageParam,
  required InfiniteQueryOptions<T, TPageParam> options,
}) =>
    NotifierProvider.family<InfiniteQueryNotifierFamily<T, TPageParam, P>,
        InfiniteQueryState<T>, P>(
      () => InfiniteQueryNotifierFamily<T, TPageParam, P>(
        queryFn: queryFn,
        options: options,
        initialPageParam: initialPageParam,
        queryKey: name,
...
/// 🔥 Auto-dispose Notifier-based infinite query provider with parameters
///
/// **Use this when:**
...
/// Example:
/// ```dart
/// final userPostsProvider = infiniteQueryProviderFamilyAutoDispose<Post, int, int>(
///   name: 'user-posts',
...
///
/// // Usage in widget:
/// final userPostsResult = ref.readInfiniteQueryResultFamilyAutoDispose(userPostsProvider, userId);
/// userPostsResult.state.when(
///   idle: () => Text('Tap to load'),
///   loading: () => CircularProgressIndicator(),
///   success: (pages, hasNextPage, hasPreviousPage, fetchedAt) =>
///     ListView.builder(
...
AutoDisposeNotifierProviderFamily<
    InfiniteQueryNotifierFamilyAutoDispose<T, TPageParam, P>,
    InfiniteQueryState<T>,
    P> infiniteQueryProviderFamilyAutoDispose<T, TPageParam, P>({
  required String name,
  required QueryFunctionWithParamsWithRef<T, TPageParam> Function(P) queryFn,
  required TPageParam initialPageParam,
  required InfiniteQueryOptions<T, TPageParam> options,
...
extension WidgetRefReadQueryResult on WidgetRef {
  ...
  InfiniteQueryResult<T> readInfiniteQueryResult<T, TPageParam>(
      NotifierProvider<InfiniteQueryNotifier<T, TPageParam>,
              InfiniteQueryState<T>>
          provider) {
  ...
  InfiniteQueryResult<T> readInfiniteQueryResultAutoDispose<T, TPageParam>(
  ...
  /// Create an infinite query result from a family provider
  InfiniteQueryResult<T> readInfiniteQueryResultFamily<T, TPageParam, P>(
      NotifierProviderFamily<InfiniteQueryNotifierFamily<T, TPageParam, P>,
              InfiniteQueryState<T>, P>
          provider,
      P param) {
  ...
  /// Create an infinite query result from a family auto-dispose provider
  InfiniteQueryResult<T>
      readInfiniteQueryResultFamilyAutoDispose<T, TPageParam, P>(
          AutoDisposeNotifierProviderFamily<
                  InfiniteQueryNotifierFamilyAutoDispose<T, TPageParam, P>,
                  InfiniteQueryState<T>,
                  P>
...
extension InfiniteQueryStateExtension<T> on InfiniteQueryState<T> {
  R when<R>({
    ...
    required R Function(List<T> pages, bool hasNextPage, bool hasPreviousPage,
            DateTime? fetchedAt)
        success,
    required R Function(Object error, StackTrace? stackTrace) error,
    // ignore: avoid_positional_boolean_parameters
    R Function(List<T> pages, bool hasNextPage, bool hasPreviousPage,
            DateTime? fetchedAt)?
        refetching,
    R Function()? idle,
    // ignore: avoid_positional_boolean_parameters
    ...
    return switch (this) {
      InfiniteQueryIdle<T>() => idle?.call()??loading(),
      InfiniteQueryLoading<T>() => loading(),
      final InfiniteQuerySuccess<T> successState => success(
          successState.pages,
          successState.hasNextPage,
          successState.hasPreviousPage,
          successState.fetchedAt),
      final InfiniteQueryRefetching<T> refetchingState => (refetching??success).call(
          refetchingState.pages,
          refetchingState.hasNextPage,
          refetchingState.hasPreviousPage,
          refetchingState.fetchedAt),
      final InfiniteQueryError<T> errorState =>
        error(errorState.error, errorState.stackTrace),
      final InfiniteQueryFetchingNextPage<T> fetching => (fetchingNextPage??success).call(
          fetching.pages,
          fetching.hasNextPage,
          fetching.hasPreviousPage,
          fetching.fetchedAt),
      final InfiniteQueryFetchingPreviousPage<T> fetching =>
        (fetchingPreviousPage??success).call(fetching.pages, fetching.hasNextPage,
            fetching.hasPreviousPage, fetching.fetchedAt),
    };
  }
}
