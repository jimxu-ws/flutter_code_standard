...
import 'state_query_provider.dart'
    show QueryFunctionWithRef, QueryFunctionWithParamsWithRef;
...
class QueryNotifier<T> extends Notifier<QueryState<T>> {
  ...
  @override
  QueryState<T> build() {
    Log.info('Building modern query notifier with cached data, $queryKey');
    if (!_isInitialized) {
      _isInitialized = true;
      _isDisposed = false;
    ...
    if (options.enabled && options.refetchOnMount) {
      Log.info(
          'Refetching on mount in modern query notifier with cached data, $queryKey');
    ...
    if (cachedEntry != null && !cachedEntry.isStale && cachedEntry.hasData) {
      return QuerySuccess(cachedEntry.data as T,
  ...
  Future<void> _fetch({bool forceFetchRemote = false}) async {
    ...
    Log.info('Fetching data in modern query notifier for key $queryKey');
    ...
    if (!forceFetchRemote &&
        ...
        !cachedEntry.isStale &&
        cachedEntry.hasData) {
      Log.info('Using cached data in modern query notifier for key $queryKey');
    ...
    if (options.keepPreviousData && state.hasData) {
      Log.info(
          'Refetching with state data in modern query notifier for key $queryKey');
      _safeState(
        ...
        cachedEntry.hasData) {
      Log.info(
          'Refetching with stale cached data in modern query notifier for key $queryKey');
      _safeState(QueryRefetching(cachedEntry.data as T,
          fetchedAt: cachedEntry.fetchedAt));
    } else {
      Log.info('Loading in modern query notifier for key $queryKey');
      _safeState(const QueryLoading());
    }

    try {
      Log.info(
          'Querying data from server in modern query notifier for key $queryKey');
  ...
  void _scheduleRefetch() {
    Log.info(
        'Scheduling automatic refetching in modern query notifier with cached data, $queryKey');
    _refetchTimer?.cancel();
  ...
  void _onWindowBlured() {
    if(isMobile()){
      Log.info('App paused in modern query notifier');
  ...
  void _onWindowFocused() {
    Log.info('Window focused in modern query notifier');
  ...
  void _setCachedEntry(QueryCacheEntry<T> entry) {
    ...
    Log.info(
        'Setting cached entry for key $queryKey in modern query notifier, change state to ${state.runtimeType}');
  }
  ...
  void _setupCacheListener() {
    Log.info(
        'Setting up cache listener in modern query notifier with cached data, $queryKey');
    _cache.addListener<T>(queryKey, (entry) {
      if ((entry?.hasData ?? false) &&
          !(state.hasData && entry!.data == state.data)) {
        Log.info(
            'Cache data changed for key $queryKey in modern query notifier, change state to ${state.runtimeType}');
        _safeState(QuerySuccess(entry!.data as T, fetchedAt: entry.fetchedAt));
      } else if (entry == null) {
        Log.info(
            'Cache entry removed for key $queryKey in modern query notifier, change state to ${state.runtimeType}');
      ...
      Log.info(
          'Cache listener called for key $queryKey in modern query notifier, change state to ${state.runtimeType}');
    });
  }
}

/// Modern Family Notifier for queries with parameters
class QueryNotifierFamily<T, P> extends FamilyNotifier<QueryState<T>, P> {
  QueryNotifierFamily({
    required this.queryFunction,
    required this.options,
    required this.queryKey,
  ...
  QueryState<T> build(P arg) {
    ...
    Log.info(
        'Building modern query notifier family with cached data, $paramKey');
    ...
    if (options.enabled && options.refetchOnMount) {
      Log.info(
          'Refetching on mount in modern query notifier family with cached data, $paramKey');
  ...
  Future<void> _fetch(P param, {bool forceFetchRemote = false}) async {
    ...
    Log.info('Fetching data in modern query notifier family for key $paramKey');
    ...
    if (!forceFetchRemote &&
        ...
        cachedEntry.hasData) {
      Log.info(
          'Using cached data in modern query notifier family for key $paramKey');
      _safeState(QuerySuccess(cachedEntry.data as T,
          fetchedAt: cachedEntry.fetchedAt));
      return;
    }
  ...
  Future<void> refetch({bool forceFetchRemote = false}) =>
      _fetch(arg, forceFetchRemote: forceFetchRemote);
  ...
  void _setupCacheListener(String key) {
    Log.info(
        'Setting up cache listener in modern query notifier family with cached data, $key');
    _cache.addListener<T>(key, (entry) {
      if ((entry?.hasData ?? false) &&
          !(state.hasData && entry!.data == state.data)) {
        Log.info(
            'Cache data changed for key $key in modern query notifier family $key');
        _safeState(QuerySuccess(entry!.data as T, fetchedAt: entry.fetchedAt));
      } else if (entry == null) {
        Log.info(
            'Cache entry removed for key $key in modern query notifier family $key');
        if (options.onCacheEvicted != null) {
      ...
      Log.info(
          'Cache listener called for key $key in modern query notifier family, change state to ${state.runtimeType}');
    });
  }
...
class QueryNotifierAutoDispose<T> extends AutoDisposeNotifier<QueryState<T>> {
...
class QueryNotifierFamilyAutoDispose<T, P>
    extends AutoDisposeFamilyNotifier<QueryState<T>, P> {
...
/// );
/// ```
AutoDisposeNotifierProvider<QueryNotifierAutoDispose<T>,
    QueryState<T>> queryProviderAutoDispose<T>({
  required String name,
  required QueryFunctionWithRef<T> queryFn,
  QueryOptions<T> options = const QueryOptions(),
}) =>
...
/// ```
NotifierProviderFamily<QueryNotifierFamily<T, P>, QueryState<T>, P>
    queryProviderWithParam<T, P>({
  required String name,
...
/// );
/// ```
AutoDisposeNotifierProviderFamily<QueryNotifierFamilyAutoDispose<T, P>,
    QueryState<T>, P> queryProviderWithParamAutoDispose<T, P>({
  required String name,
  required QueryFunctionWithParamsWithRef<T, P> queryFn,
  QueryOptions<T> options = const QueryOptions(),
}) =>
    NotifierProvider.autoDispose
        .family<QueryNotifierFamilyAutoDispose<T, P>, QueryState<T>, P>(
