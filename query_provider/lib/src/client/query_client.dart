...
QueryCache getCache({QueryCacheStrategy strategy = QueryCacheStrategy.memory}) {
  if (QueryGlobalOptions().storage == null) {
    Log.warn('QueryClient: Storage is not set, using memory cache');
    return MemoryQueryCache();
  }
  switch (strategy) {
    case QueryCacheStrategy.memory:
      return MemoryQueryCache();
    case QueryCacheStrategy.disk:
      return DiskQueryCache();
    case QueryCacheStrategy.disk2Level:
      return Disk2LevelQueryCache();
  }
}
...
class QueryClient {
  QueryClient({
    ...
    QueryCache? cache,
        ...
        _cache = cache ?? MemoryQueryCache();

  // Private constructor for singleton
  QueryClient._internal()
      : _container = null,
        _cache = MemoryQueryCache();
  ...
  void invalidateQueries(String keyPattern, {bool markAsStale = false}) {
    ...
    if (_container == null) {
      Log.info(
          '⚠️ QueryClient: Provider invalidation not available. Only cache cleared for pattern: $keyPattern');
      return;
    }

    // Find all providers that match the pattern and invalidate them
    final matchingKeys = _container!
        .getAllProviderElements()
        .where(
            (element) => element.provider.name?.contains(keyPattern) ?? false)
    ...
    Log.info(
        '✅ QueryClient: Invalidated ${matchingKeys.length} providers for pattern: $keyPattern');
  }
  ...
  void invalidateAll() {
    ...
    if (_container == null) {
      Log.info(
          '⚠️ QueryClient: Provider invalidation not available. Only cache cleared.');
      return;
    }
    ...
    Log.info(
        '✅ QueryClient: Invalidated all ${elements.length} providers and cleared cache.');
  }
...
final memoryQueryClientProvider = Provider<QueryClient>((ref) {
